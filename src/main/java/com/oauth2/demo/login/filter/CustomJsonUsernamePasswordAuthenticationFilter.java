package com.oauth2.demo.login.filter;
...
public class CustomJsonUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    private static final String DEFAULT_LOGIN_REQUEST_URL = "/login"; // 로그인 요청 URL
    private static final String HTTP_METHOD = "POST"; // 로그인 요청 HTTP 메소드
    ...
    private static final AntPathRequestMatcher DEFAULT_LOGIN_PATH_REQUEST_MATCHER =
            new AntPathRequestMatcher(DEFAULT_LOGIN_REQUEST_URL, HTTP_METHOD); // 로그인 요청 URL, HTTP 메소드를 가지고 AntPathRequestMatcher 생성

    private final ObjectMapper objectMapper; // JSON 데이터를 객체로 매핑해주는 객체

    public CustomJsonUsernamePasswordAuthenticationFilter(ObjectMapper objectMapper) {
        super(DEFAULT_LOGIN_PATH_REQUEST_MATCHER); // 로그인 요청 URL, HTTP 메소드를 가지고 AntPathRequestMatcher 생성
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException {
        ...
        Map<String, String> usernamePasswordMap = objectMapper.readValue(messageBody, Map.class); // JSON -> Map 변환

        String email = usernamePasswordMap.get(USERNAME_KEY);
        String password = usernamePasswordMap.get(PASSWORD_KEY);

        UsernamePasswordAuthenticationToken authRequest = new UsernamePasswordAuthenticationToken(email, password); // UsernamePasswordAuthenticationToken 객체 생성
